# Rucking Fitness News — Issue 01

**Issue title:** Neil Neil's Mystic Peel  
**Format:** Interactive palm reader / web newsletter  
**Suggested live domain:** `news.ruckingfitness.co.uk`

## Deploy to Netlify

This is a static site. No build command is required.

- Publish directory: `.`
- Entry page: `index.html`

You can either:
1. Upload this folder/repository to GitHub and connect the repository to Netlify, or
2. Drag the site folder into Netlify for a manual deploy.

## Files

- `index.html` — the complete interactive Issue 01 page
- `assets/palm-reader.jpg` — palm reader artwork
- `netlify.toml` — Netlify static-site settings

## Future issues

`news.ruckingfitness.co.uk` can remain the permanent home for Rucking Fitness News.
Issue 01 is Neil Neil's Mystic Peel; later issues can use different concepts without needing a new subdomain.

## Issue 01 v1.1 update

- Added the approved Rucking Fitness mountain/wordmark site icon and home-screen icons.
- Fixed the malformed `DOCTYPE` that was appearing as text at the top of the page.
- Enlarged the palm-reader presentation on mobile by reducing unused side margins.
- Added a clear five-colour legend underneath the palm so the key remains readable on phones.
- Preserved all 26 interactive hotspots and Issue 01 content.

## Issue 01 v1.2 update

- Preserved all 26 original palm hotspots and their existing information.
- Replaced the small lower legend with five large circular line buttons.
- Challenge, Community, Fitness, Adventure and Future buttons are now tappable.
- Each large line button opens the corresponding existing line information from Neil's hotspot data using the same modal as the palm.
- Enlarged the “26 TAPPABLE POINTS” message for mobile.
- No individual hotspot copy or category data was removed or rewritten.
